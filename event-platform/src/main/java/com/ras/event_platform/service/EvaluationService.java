...
public class EvaluationService {
    ...
    public Evaluation gradeSubmission(Long judgeId, EvaluationRequest request) {
        ...
        // 2. Validate Game State (Allow PENDING, GRADED, APPROVED, REJECTED if
        // upserting)
        // Only block if we're trying to evaluate something completely invalid.
        // Actually, we shouldn't block upserts based on PENDING anymore since they can
        // edit APPROVED submissions.

        // 3. Calculate Total Score mathematically on the backend (Average of
        // categories)
        ...
        Optional<Evaluation> existingOpt = evaluationRepository.findBySubmissionIdAndJudgeId(submission.getId(),
                judgeId);
            ...
            if (newAverage >= passingThreshold) {
                ...
                if (combinedFeedback.isEmpty()) {
                    combinedFeedback = "Your submission scored " + newAverage
                            + " points. This did not meet the required event threshold of " + passingThreshold
                            + " points.";
                }
